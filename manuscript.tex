...
Modeling and control of flapping wing MAVs is becoming a well studied subject\cite{floreano:flying}. 
Shigeoka discusses velocity and altitude control in ~\cite{Shigeoka:ornithopter}. 
Design and control of the sub-15 gram DelFly ornithopter is described by 
deCroon et al.~\cite{delfly:design}. In contrast, there is little previous work 
on guidance of flapping wing MAVs. Baek demonstrated altitude control with an external 
...
the non-linear flight dynamics~\cite{humbert:dipteran2}\cite{cheng:tandrdamping}. 
One approach to simplify these dynamic models is to use averaging over the wingbeat
...
Many of the methods for controlling these platforms make extensive use of GPS in outdoor
environments and expansive motion capture systems in indoor environments
