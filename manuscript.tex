...
Modeling of the vehicle pose is divided into two independent models for simplicity: one for
yaw motion, and one for translational motion.

We model the yaw motion of the ornithopter with a one-dimensional model to reduce the complexity of the overall model. Matching our physical implementation, we choose the reference heading angle as an input and model the dynamics from the vehicle and on-board PID controller to output the physical heading angle. The model format is shown in Equation~\ref{eq:plain_model}.
...
experimentally by measuring the response of the system to the step input of a
...
\label{sec:experiments:verification}

We tested our cooperative visual servoing system with a simple target seeking
experiment: H$^2$Bird must fly through a specified obstacle, in this case,
a wooden frame meant to simulate a window. As H$^2$Bird cannot detect the
target on its own, we used the ground station to provide remote headings for
navigation to the window. A VICON~\footnote{Vicon:~\url{http://www.vicon.com/}} motion capture system in the test room provided
ground truth data. A sketch of the experimental setup is shown in
Fig.~\ref{fig:experiment_cartoon} and a screen capture from the camera perspective is in Fig.~\ref{fig:pf_screencap}, pane 1.

% TODO clarify or roll into paragraph
Each trial included the following steps:
\begin{enumerate}
\item The window is identified by the user (Fig.~\ref{fig:pf_screencap}, pane 2). 
\item The H$^2$Bird is released in view of the camera by hand at the desired starting grid point (Fig.~\ref{fig:pf_screencap}, pane 3).
\item The H$^2$Bird attempts to fly to the window with remote guidance from
the base station (Fig.~\ref{fig:pf_screencap}, panes 4,5,6).
\end{enumerate}
...
%TODO cameron
The results of our system identification of the cooperative visual servoing model are detailed in the following sections. We use the model to empirically and analytically predict the feasibility of the cooperative control system for various initial conditions. Furthermore, we validate the model using experimental trials.
...
The step response rise time is 1.4 seconds. H$^2$Bird flies
at an average forward velocity of 1.2 m/s during our experiments, so the
estimated minimum turning radius of the robot is 1.07 meters, as described in
Equation~\ref{eq:min_radius}. In the experiments, we choose a slower flight speed
