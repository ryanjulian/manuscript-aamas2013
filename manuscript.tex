...
gyroscope-based control with a lightweight ground station design, which has power and weight 
...
the ornithopter to reliably negotiate flight through narrow openings. We demonstrate the performance of the location estimation and navigation  algorithms.
\end{abstract}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ACM METADATA
...
significantly limit sensing their and computational capabilities. They can execute only the most rudimentary
control and state estimation algorithms in real-time. 
The constraints on flapping-wing MAVs are even more restrictive. 
Though they exhibit demonstrate safety and noise profiles superior to those
of rotor craft MAVs, this comes at the expense of reduced payload weight.

The non-linear
flight dynamics exhibited of flapping wing flight compounding constraints~\cite{humbert:dipteran2}\cite{cheng:tandrdamping}. 
Much past research on flapping wing MAVs focused on overcoming the 
difficulties associated with controlling and modeling flapping-wing vehicles in the face of this
...
\cite{cheng:tandrdamping}, and using linear, low-dimensional models to predict flight behavior~\cite{Shigeoka:ornithopter}. These advances lead to the
development of some high-performance platforms, such as the sub-15 gram DelFly 
described by deCroon et al.~\cite{delfly:design}. 
...
control methods described %TODO you need to use real names, it's not kosher to refer to citation numbers directly
...
To model the translational position of the ornithopter, we use a form of Dubin's car model (Eq.~\ref{eq:total_model_plain})~\cite{lavalle:planning}. 
\begin{equation}
...
explicitly fact that the pose estimation algorithm does not estimate the depth of the
robot.
...
We performed each trial as follows (see Fig.~\ref{fig:pf_screencap}):
\begin{enumerate}
...
passage traversal, using both analytical and empirical methods. We then conducts 
...
We present the results of our experimental trials in
...
we observed in the experimental data. We also account for the camera capture and
