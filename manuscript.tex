...
vehicles or analytical explorations. Hyams et al., Jung et al. and Nebot et al. all 
demonstrated cooperative visual servoing for heterogeneous UGVs~\cite{Hyams1999Cooperative}\cite{Jung1998Range}\cite{Nebot2003Agents}. 
Luo, et al. and Mehta, et al. analyze  UAV-UGV cooperation from a modeling perspective~\cite{Luo2011Air}\cite{Mehta2006Adaptive}. 
Rudol et al. demonstrated a similar cooperative visual servoing system with 
UGVs and MAVs~\cite{Rudol2008Micro}, but the vehicles used were orders of 
magnitude larger than the lightweight systems we target, and so were able 
...
station then uses a PID control law to calculate a new reference attitude, 
...
We validate our system model experimentally by systematically attempting to
guide the H$^2$Bird through the window using the directed headings from the
ground station at various starting initial conditions. We compare the results of the
experiments to the computed backwards reachable set of feasible initial
conditions and a Monte Carlo simulation to determine the accuracy of our system 
model.
...
starting grid point.
\item The H$^2$Bird attempts to fly through the window, with remote 
